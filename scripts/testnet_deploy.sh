...
near create-account $MARKET --masterAccount $CONTRACT_NAME --initialBalance 50
near deploy $MARKET ./target/wasm32-unknown-unknown/release/nft_market.wasm --masterAccount $CONTRACT_NAME
near call $MARKET init '{"nft_id": "'$CONTRACT_NAME'"}' --accountId $CONTRACT_NAME
#
near call $CONTRACT_NAME nft_approve '{"token_id": "1", "account_id": "'$MARKET'", "msg": "{\"sale_type\":\"selling\",\"price\":\"10\"}"}' --accountId $CONTRACT_NAME --depositYocto 510000000000000000000

export NEW_OWNER_ID=alice.$CONTRACT_NAME
near create-account $NEW_OWNER_ID --masterAccount $CONTRACT_NAME --initialBalance 30
near call $MARKET buy '{"token_id": "1"}' --depositYocto 10 --gas 90000000000000 --accountId $NEW_OWNER_ID
