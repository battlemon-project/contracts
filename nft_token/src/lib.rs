...
use near_sdk::{
    env, near_bindgen, require, AccountId, BorshStorageKey, PanicOnDefault, PendingContractTx,
...
impl Contract {
    ...
    pub fn mint(
        ...
        let (metadata, model) = token_metadata.split();
...
mod tests {
    ...
    use nft_models::lemon::{Lemon, Type as LemonType};
    ...
    #[test]
    fn nested_tokens_must_return_self() {
        let mut contract = Contract::init(alice());
    ...
    fn nested_tokens_must_return_self_and_two_weapons_and_right_weapon_suppressor() {
        ...
        let suppressor = Suppressor { parent: None };

        let left_weapon = get_foo_weapon();
        let right_weapon = Weapon {
            level: 1,
            r#type: WeaponType::Projection,
            barrel_slot: Some(suppressor_token_id.clone()),
            ..left_weapon.clone()
        };
    ...
    fn lemon_nft_transfer_change_owner_only_for_lemon_token_and_disassemble_it_from_weapon_nft() {
        ...
        lemon.left_weapon_slot = Some(weapon_id.clone());
